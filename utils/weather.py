...
def display_location_weather(location):
    ...
    if  search == None:
        st.write(f"{location} is not found in our database, please check spelling and try again.")
        return None
    elif len(search)> 1:
        select_match = st.selectbox("Multiple matching locations found, please select:", [(i,row[2],row[1],row[3]) for i,row in enumerate(search)])
