...
def start_outdoor_session(activity_type):
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS reminder (date VARCHAR(30),email VARCHAR(30),activity VARCHAR(30))"
        )
        cursor.execute(
            "INSERT INTO reminder (date,email,activity) VALUES (%s,%s,%s)",
            (reminder["start"]["dateTime"], email, activity_type),
        )
    connection.commit()
    connection.close()
    return email
